...
![数据结构示例](../figures/screenshots/ss-01.png)
...
![描述性统计表](../figures/screenshots/ss-02.png)
...
![Kruskal-Wallis检验结果](../figures/screenshots/ss-03.png)
...
![Games-Howell事后检验结果](../figures/screenshots/ss-04.png)
...
![年龄组死亡率均值图（含误差棒）](../figures/screenshots/ss-05.png)

![年龄组死亡率分布箱线图](../figures/screenshots/ss-06.png)
...
| 成员        | 角色                     | 主要贡献                                                                                                                                     |
| ----------- | ------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------- |
| **安涛**    | Team Lead & Technical    | • 项目整体架构设计 • Python框架搭建（`run_analysis.py`） • 数据处理模块开发（`data_processing.py`） • 代码集成与调试 • Git仓库管理与版本控制 |
| **毛同学** | Technical                | • 统计分析模块实现（`statistical_analysis.py`） • ANOVA分析与Welch校正 • 假设检验代码编写 • 结果验证与优化                                   |
| **陈宝成**  | Technical                | • 非参数检验实现（Kruskal-Wallis） • 事后检验开发（Games-Howell） • 相关性分析模块 • 卡方独立性检验                                          |
| **余晗**    | Research & Documentation | • 文献调研与理论支撑 • WHO数据源研究 • 研究背景撰写 • 参考文献整理 • 研究局限性分析                                                          |
| **李春旭**  | Analysis & Visualization | • 可视化模块开发（`visualize_results.py`） • 统计图表生成（3张核心图表） • JASP软件分析执行 • 结果解释与报告撰写                             |

### 协作方式
- **代码协作**：GitHub分支管理，Pull Request代码评审
- **分析验证**：JASP（李春旭）与Python（毛XX、陈宝成）双重验证
